...
class ConfigurationManager:
    ...
    def load_all_configurations(
        cls, config_file: str = "config.json"
        ...
        for config_dict in config_dicts:
            ...
            if "sql_file" in config_dict:
                config_dict["sql_creation"] = cls._load_sql_file(
                    config_dict["sql_file"]
                )
    ...
    def _load_sql_file(cls, sql_filename: str) -> str:
        ...
        if "fixtures/" in sql_filename:
            sql_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "tests", sql_filename
            )
        else:
            # Standard production SQL file
            sql_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "sql", sql_filename
            )
