...
class TestEndToEndPipelines:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("consumption_config_fixture", "mock_consumption_api_data", 3, does_not_raise()),
            ("stations_config_fixture", "mock_stations_api_data", 3, does_not_raise())
        ]
    )
    def test_complete_data_pipeline_workflow(self, request, temp_db_file, temp_json_file,
                                           config_fixture, api_data_fixture, expected_records, expectation):
        ...
        with expectation:
            ...
            with DuckDBConnection(temp_db_file) as conn:
                ...
                count_result = conn.execute(f"SELECT COUNT(*) FROM {config.table_name}").fetchone()
                assert count_result[0] == expected_records
                
                # Step 6: Verify specific data content
                all_data = conn.execute(f"SELECT * FROM {config.table_name} ORDER BY id").fetchall()
                assert len(all_data) == expected_records
                assert all_data[0][0] == 1  # First record ID should be 1

    def test_consumption_pipeline_data_integrity(self, temp_db_file, temp_json_file, 
                                                consumption_config_fixture, mock_consumption_api_data):
        ...
        with DuckDBConnection(temp_db_file) as conn:
            ...
            conn.execute(config.sql_creation)
            conn.execute(f"INSERT INTO {config.table_name} SELECT * FROM read_json_auto('{temp_json_file}')")
            
            # Verify consumption calculations
            avg_consumption = conn.execute(f"SELECT AVG(consumption) FROM {config.table_name}").fetchone()[0]
            expected_avg = (150.5 + 200.3 + 175.8) / 3
            assert abs(avg_consumption - expected_avg) < 0.01
            
            # Verify date range
            date_range = conn.execute(f"SELECT MIN(date), MAX(date) FROM {config.table_name}").fetchone()
            assert date_range[0] == "2024-01-01"
            assert date_range[1] == "2024-01-03"
            
            # Verify specific consumption values
            high_consumption = conn.execute(f"SELECT * FROM {config.table_name} WHERE consumption > 200").fetchall()
            assert len(high_consumption) == 1
            assert abs(high_consumption[0][1] - 200.3) < 0.01

    def test_stations_pipeline_geographic_data(self, temp_db_file, temp_json_file,
                                              stations_config_fixture, mock_stations_api_data):
        ...
        with DuckDBConnection(temp_db_file) as conn:
            ...
            paris_data = conn.execute(f"SELECT * FROM {config.table_name} WHERE ville = 'Paris'").fetchone()
            assert abs(paris_data[1] - 48.8566) < 0.01  # latitude
            assert abs(paris_data[2] - 2.3522) < 0.01   # longitude
            
            # Verify city count
            city_count = conn.execute(f"SELECT COUNT(DISTINCT ville) FROM {config.table_name}").fetchone()[0]
            assert city_count == 3
    ...
    def test_pipeline_error_scenarios(self, temp_db_file, consumption_config_fixture, error_scenario, expectation):
        ...
        with expectation:
            with DuckDBConnection(temp_db_file) as conn:
                if error_scenario == "missing_json_file":
                    ...
                    conn.execute(config.sql_creation)
                    conn.execute(f"INSERT INTO {config.table_name} SELECT * FROM read_json_auto('nonexistent.json')")
                    ...
                    conn.execute(f"INSERT INTO wrong_table_name VALUES (1, 'test')")

    def test_multi_pipeline_database_sharing(self, temp_db_file, temp_json_file,
                                           consumption_config_fixture, stations_config_fixture,
                                           mock_consumption_api_data, mock_stations_api_data):
        ...
        with DuckDBConnection(temp_db_file) as conn:
            ...
            conn.execute(f"INSERT INTO {consumption_config.table_name} SELECT * FROM read_json_auto('{temp_json_file}')")
            
            # Load stations data (overwrite temp file)
            save_json(mock_stations_api_data["results"], temp_json_file)
            conn.execute(f"INSERT INTO {stations_config.table_name} SELECT * FROM read_json_auto('{temp_json_file}')")
            
            # Verify both tables have data
            consumption_count = conn.execute(f"SELECT COUNT(*) FROM {consumption_config.table_name}").fetchone()[0]
            stations_count = conn.execute(f"SELECT COUNT(*) FROM {stations_config.table_name}").fetchone()[0]
