...
from utils.api import fetch_paginated_api, fetch_with_pagination
...
class TestApi:
    ...
    @pytest.mark.parametrize(
        "mock_response_fixture,pagination_key,data_key,expected_total",
        [
            pytest.param(
                "mock_api_success_response", "next", "data", 1, id="success_response"
            ),
            pytest.param(
                "mock_empty_api_response", "next", "data", 0, id="empty_response"
            ),
        ],
    )
    def test_fetch_with_pagination_fixtures(
        self, request, mock_response_fixture, pagination_key, data_key, expected_total
    ):
        ...
        with patch("requests.get") as mock_get:
            ...
            result = fetch_with_pagination(
                "http://api.test", pagination_key=pagination_key, data_key=data_key
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                requests.RequestException("Network error"), id="network_error"
            ),
            pytest.param(
                requests.ConnectionError("Connection failed"), id="connection_error"
            ),
    ...
    def test_api_integration_with_config_fixture(self, valid_economie_gouv_config):
        ...
        mock_response.json.return_value = {
            "results": [{"id": 1, "latitude": 48.8566, "longitude": 2.3522}],
            "total_count": 1,
        }
        mock_response.raise_for_status.return_value = None
    ...
    def test_fetch_paginated_api_no_results(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            result = fetch_paginated_api(
                "http://api.test?limit={limit}&offset={offset}"
            )
