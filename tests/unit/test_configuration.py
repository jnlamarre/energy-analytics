...
class TestConfigurationObjects:
    ...
    def test_economie_gouv_url_without_select_fields(self, economie_gouv_empty_select):
        ...
        assert (
            "select=" not in url
            or "select=" in url
            and url.split("select=")[1].split("&")[0] == ""
        )
    ...
    def test_economie_gouv_select_field_variations(
        self, config_factory, select_fields, expected_in_url
    ):
    ...
    def test_configuration_object_attributes(
        self, valid_economie_gouv_config, valid_data_gouv_config
    ):
...
class TestConfigurationManager:
    ...
    def test_load_configurations_success(
        self, valid_economie_gouv_list, valid_data_gouv_list
    ):
    ...
    def test_individual_configurations(
        self, valid_economie_gouv_list, valid_data_gouv_list
    ):
        ...
        with patch.object(ConfigurationManager, "load_all_configurations") as mock_load:
            ...
            mock_configs = [
                DataGouvConfiguration(
                    ...
                    sql_file="test1.sql",
                    sql_creation="CREATE TABLE test1",
                    table_name="consumption",
                ),
                EconomieGouvConfiguration(
                    api_type="economie_gouv",
                    dataset="test",
                    target_file="data2.json",
                    sql_file="test2.sql",
                    sql_creation="CREATE TABLE test2",
                    table_name="stations",
                    select=[],
                ),
            ]
            ...
            data_gouv_configs = ConfigurationManager.get_configurations_by_api_type(
                "data_gouv"
            )
            assert len(data_gouv_configs) == 1
            assert data_gouv_configs[0].api_type == "data_gouv"

            economie_configs = ConfigurationManager.get_configurations_by_api_type(
                "economie_gouv"
            )
    ...
    def test_load_configurations_with_unsupported_api_type(self):
        ...
        try:
            with (
                ...
                with pytest.raises(
                    ValueError, match="Unsupported API type: unsupported_api"
                ):
