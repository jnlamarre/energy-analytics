...
class TestStationsStorage:
    ...
    def test_create_table_with_config_sql(self):
        ...
        mock_config_manager.get_configuration_by_table.assert_called_once_with('stations')
    ...
    def test_insert_data_successful(self):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            mock_file.assert_called_once_with("/path/to/stations.json", 'r', encoding='utf-8')
            ...
            # Verify logging
            mock_logger.info.assert_any_call("Loading station data from /path/to/stations.json...")
    ...
    def test_insert_data_file_not_found(self):
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            with pytest.raises(FileNotFoundError, match="Data file not found: /nonexistent.json"):
    ...
    def test_insert_data_empty_file(self):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            mock_logger.warning.assert_called_once_with("No data to import (empty JSON file)")
    ...
    def test_insert_data_corrupted_file(self):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            mock_logger.error.assert_called_once_with("Cannot read data file or file is corrupted")
...
class TestStationsPipeline:
    ...
    def test_fetch_successful(self):
        ...
        with patch('pipelines.stations.ConfigurationManager') as mock_config_mgr, \
            ...
            mock_config_mgr.get_configuration_by_table.assert_called_once_with('stations')
            
            # Verify API call with URL template conversion
            expected_url = "http://api.test?limit={limit}&offset={offset}"
            mock_fetch.assert_called_once_with(expected_url, limit=100, max_records=10000)
    ...
    def test_fetch_no_configuration_found(self):
        ...
        with patch('pipelines.stations.ConfigurationManager') as mock_config_mgr:
            ...
            with pytest.raises(ValueError, match="No configuration found for stations table"):
                pipeline.fetch()
