...
from pipelines.consumption import (
    ConsumptionPipeline,
...
class TestConsumptionProcessor:
    ...
    def test_process_data_passthrough(self):
        ...
        test_data = [{"id": 1, "consumption": 100}, {"id": 2, "consumption": 200}]
...
class TestConsumptionPipeline:
    ...
    def test_fetch_single_date_query(self):
        ...
        with (
            patch("pipelines.consumption.ConfigurationManager") as mock_config_mgr,
            ...
            mock_fetch.assert_called_once_with(
                "http://api.test", {"Date__exact": '"2026-01-15"'}
            )

            # Verify logging
            mock_logger.info.assert_any_call("Fetching consumption data...")
            mock_logger.info.assert_any_call(
                "Fetching data for single date: 2026-01-15"
            )
    ...
    def test_fetch_date_range_with_defaults(self):
        ...
        with (
            ...
            mock_fetch.assert_called_once_with(
                "http://api.test",
                {"Date__greater": "2026-01-01", "Date__less": "2026-03-31"},
            )

            # Verify logging
            mock_logger.info.assert_any_call(
                "Fetching data for date range: 2026-01-01 to 2026-03-31"
            )
    ...
    def test_fetch_date_range_custom(self):
        ...
        with (
            ...
            mock_fetch.assert_called_once_with(
                "http://api.test",
                {"Date__greater": "2026-02-01", "Date__less": "2026-02-28"},
            )
    ...
    def test_fetch_no_configuration_found(self):
        ...
        with patch("pipelines.consumption.ConfigurationManager") as mock_config_mgr:
            ...
            with pytest.raises(
                ValueError, match="No configuration found for consumption table"
            ):
                pipeline.fetch()
    ...
    def test_fetch_api_exception_handling(self):
        ...
        with (
            ...
            mock_logger.error.assert_called_once_with(
                "Error fetching consumption data: API Error"
            )
