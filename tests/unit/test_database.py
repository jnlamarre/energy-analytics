...
class TestDatabase:
    ...
    def test_json_data_loading(
        self, temp_database, test_json_data, database_operation_test_data
    ):
    ...
    def test_empty_json_handling(self, temp_database, empty_json_data):
        ...
        with DuckDBConnection(temp_database) as conn:
            try:
                ...
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS empty_test (id INTEGER, value TEXT)"
                )
                result = conn.execute("SELECT COUNT(*) FROM empty_test").fetchone()
                assert result[0] == 0
    ...
    def test_error_scenarios(self, temp_database, invalid_json_data):
        ...
        with DuckDBConnection(temp_database) as conn:
            ...
            with pytest.raises(Exception) as exc_info:
                conn.execute(
                    "INSERT INTO error_test SELECT * FROM read_json_auto('nonexistent_file.json')"
                )
            ...
            assert (
                ...
                or "no files found" in error_message
            )

            # Test invalid JSON error
            with pytest.raises(Exception):
                conn.execute(
                    f"INSERT INTO error_test SELECT * FROM read_json_auto('{invalid_json_data}')"
                )
    ...
    def test_parametrized_table_operations(
        self, temp_database, database_operation_test_data
    ):
        ...
        for table_type, test_data in database_operation_test_data.items():
            with DuckDBConnection(temp_database) as conn:
                ...
                count_result = conn.execute(
                    f"SELECT COUNT(*) FROM {test_data['table_name']}"
                ).fetchone()
