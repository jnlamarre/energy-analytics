...
class TestIntegration:
    ...
    def test_configuration_to_database_pipeline(
        self, temp_database, valid_data_gouv_config
    ):
    ...
    def test_file_config_database_integration(
        self, temp_database, mixed_configs_list, tmp_path
    ):
        ...
        with patch.object(
            ConfigurationManager, "_load_sql_file", return_value="SELECT 1"
        ):
            # Load configs back from file
            loaded_configs = ConfigurationManager.load_all_configurations(
                str(config_file)
            )
    ...
    def test_api_mock_to_file_to_database(
        self,
        temp_database,
        mock_paginated_responses,
        valid_economie_gouv_config,
        tmp_path,
    ):
    ...
    def test_configuration_manager_with_database_fixtures(
        self,
        ...
        with DuckDBConnection(database_with_test_data) as conn:
            ...
            result = conn.execute(
                "SELECT * FROM test_consumption ORDER BY id"
            ).fetchall()
            assert len(result) == 2
            assert result[0][0] == 1
    ...
    @pytest.mark.parametrize(
        "config_fixture,expected_api_type",
        [
            pytest.param(
                "valid_economie_gouv_config", "economie_gouv", id="economie_integration"
            ),
            pytest.param("valid_data_gouv_config", "data_gouv", id="data_integration"),
        ],
    )
    def test_parametrized_integration(
        self, request, temp_database, config_fixture, expected_api_type
    ):
        ...
        with DuckDBConnection(temp_database) as conn:
            ...
            # Insert test record
            conn.execute(
                f"INSERT INTO {config.table_name}_test VALUES (1, '{config.api_type}')"
            )
